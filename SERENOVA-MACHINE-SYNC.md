...
| laptop | **EventDetails (web) view cleanup/redesign** + inline Guest List mgmt | **in progress** (2026-06-21) | cleaner view/flow; today `GuestListCard` is approve/deny-only, full `GuestListTab` coupled to EditEvent state → extract into a self-contained component. Recon done (EventDetails.jsx ~827 lines, right-rail card stack of ~8 cards). Gathering owner's design direction (pain points + guest-mgmt depth) before building. |
