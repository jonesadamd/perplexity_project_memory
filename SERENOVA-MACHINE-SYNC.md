...
| Pending Base44 DEPLOY (owner) | **#7c Part B** — redeploy `getVenueRouteInfo` + sync `Accommodation` (`route_to_airport` field). Until then the hotel→airport times stay hidden; everything else (#7c Part A all-hotels drive times, #10 follow-ups) works on a plain rebuild. |
...
| desktop | EventDetails **#7c** — >1-hotel driving-only + hotel→airport leg | **✅ DONE 2026-06-22** (`0.6.0572`→`0.6.0573`) | **Part A** (`0.6.0572`, `2d3b9bc`, frontend-only): venue card "+N more" → toggle → all-hotels list w/ drive-time-to-venue (cached `getVenueRouteInfo` per hotel). **Part B** (`0.6.0573`, `7f761ec`): `getVenueRouteInfo` now also computes **hotel→airport** (driving), cached as new additive `Accommodation.route_to_airport`; airport resolved server-side from the event's flights (departure_iata of latest-departing flight; fallback most-frequent). Shown in primary readout + all-hotels list (Plane icon). **⚠️ Part B needs a Base44 deploy: `getVenueRouteInfo` re-provision + `route_to_airport` field sync** (Part A works without it). |
