...
> Last Updated: 2026-06-22T21:05 EDT
...
| What | Value |
|---|---|
| Code repo `main` HEAD | `0497a6a` — "MobileHub: scope travel/hotels to the band/crew member's own items" |
| Serenova version | **0.6.0577** |
...
| desktop | **MobileHub — band/crew see only their OWN travel/hotels** (Travel tab, Hotel tab, home day-schedule/week-strip); event detail pins+highlights own, collapses others | **✅ DONE 2026-06-22** (`0.6.0577`, `0497a6a`) | New `src/utils/personalTravelScope.js` (match: flights `user_email`, ground `travelers[].user_email`, hotels `rooming_list[].occupants[].email`). Applies to `band_member`/`crew_member`/`roadie_member` only; mgmt/TM keep full view; events stay rostered. **Hotels per-event fallback** (own room if matched else event hotel — owner decision, rooming lists often name-only). Event detail: all info shown, own pinned + teal "You" tag, others behind "Show N other …" (`OthersDisclosure`); no match → all plain. Frontend-only. ⏳ live-verify. |
