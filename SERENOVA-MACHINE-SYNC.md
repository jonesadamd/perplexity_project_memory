...
| What | Value |
|---|---|
| Code repo `main` HEAD | `84d5a59` — "EventDetails Stage 4: group Travel-tab accommodations by hotel" |
| Serenova version | **0.6.0569** |
...
| Pending live-verify | — (EventDetails redesign `0.6.0541`→`0.6.0569` reviewed live throughout; all gated bits deployed + verified). |
...
| **desktop** | EventDetails **#10** — Set List as a popup button (small; mirror the guest popup pattern) + tidy: remove unused `getStatusBadge`/`getContractBadge` from `EventDetails.jsx` | 🔨 **in progress (desktop) 2026-06-22** | claimed from `0.6.0569` |
| _(unclaimed)_ | EventDetails **#7c** — all-hotels-on-map / >1-hotel driving-only + hotel→airport leg | later | from the #7 working-doc "multiple hotels" note |
