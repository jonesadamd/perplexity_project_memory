...
> **👉 LAPTOP NEXT (owner priority):** see `LAPTOP-SESSION-NEXT.md` — (1) MobileHub home install/
> notifications nudge, (2) live flight updates in MobileHub (Lisa flies 2026-06-23). Both unclaimed below.
...
| What | Value |
|---|---|
| Code repo `main` HEAD | `d537c5a` — "MobileHub: venue contacts now read from venue.contacts" |
| Serenova version | **0.6.0575** |
...
| _(unclaimed — ⭐ OWNER PRIORITY for laptop, tonight/tomorrow)_ | **MobileHub home nudge** — "Add to Home Screen" (if not a PWA) + "Enable notifications" (if PWA & not granted); dismissible, reappear ~every 3 days | next | **Full spec: `LAPTOP-SESSION-NEXT.md` Item 1.** Reuse `useInstallPrompt` + `src/lib/push.js` + the hamburger menu's handlers; new `MobileHomeNudge` at top of the Home tab; localStorage 3-day suppress. Live-only on app.serenovahub.com. |
